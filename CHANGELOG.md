...
### Changed
- **Renamed the PyPI distribution** from `sdlc-kit` to `local-sdlc-kit`. The original name was rejected by PyPI's
  name-similarity check (`sdlckit` on production PyPI, by atishio/sdlc-kit, normalises to the same string). The
  console script is now `local-sdlc-kit-install`. The Python import name (`sdlc_kit`), the GitHub repository
  (`senssei/local-sdlc-kit`) and the docs site (`senssei.github.io/local-sdlc-kit/`) are unchanged.
...
- **PyPI publication.** `pip install local-sdlc-kit` lands an installable wheel that ships the same `sdlc_kit/install.py`,
  `sdlc_kit/sdlc_check.py`, and `sdlc_kit/template/` as a git clone. The installer is exposed as the
  `local-sdlc-kit-install` console script. A top-level `install.py` shim keeps `python3 install.py` working from a clone.
...
- **Documentation site** (MkDocs, `docs/` + `mkdocs.yml`): install, process, harnesses, the gate and releasing. Built with
  ...
  (`pip install local-sdlc-kit[docs]` is for maintainers; not a runtime dependency).
...
### Notes
- The kit ships as a wheel and an sdist; `pip install local-sdlc-kit` gives the user the same `local-sdlc-kit-install` CLI as a
